...
stop_anlysis['NewTaz_x']=round(stop_anlysis['NewTaz_x']/100,0)
...
origins_map_work_das=das_i_stops_work.groupby('stop_zone')['person_id'].count()
origins_map_shop_das=das_i_stops_shop.groupby('stop_zone')['person_id'].count()
origins_map_edu_das=das_i_stops_edu.groupby('stop_zone')['person_id'].count()
origins_map_other_das=das_i_stops_other.groupby('stop_zone')['person_id'].count()
...
axe.set_title('Linear regression - intermediate stops to Work - Destantion ' ,fontsize=25)
axe.set_xlabel('SimMobility ' , fontsize=25,labelpad=15)
axe.set_ylabel('THS ', fontsize=25,labelpad=15)
axe.set_xlim(0, 60000)
axe.set_ylim(0, 60000)
axe.text(40000, 10000, 'R-squared = %0.2f' % r2_score(Y1, Y1_pred),size=17)
axe.text(40000, 20000, 'Coefficients = %0.2f' % linear_regressor1.coef_,size=17)
...
axe2.set_title('Linear regression - intermediate stops to Education - Destantion ' ,fontsize=25)
axe2.set_xlabel('SimMobility ' , fontsize=25,labelpad=15)
axe2.set_ylabel('THS ', fontsize=25,labelpad=15)
axe2.set_xlim(0, 7000)
axe2.set_ylim(0, 7000)
axe2.text(2000 ,5000, 'R-squared = %0.2f' % r2_score(Y1, Y1_pred),size=17)
axe2.text(2000, 4000, 'Coefficients = %0.2f' % linear_regressor1.coef_,size=17)
...
axe3.set_title('Linear regression - intermediate stops to Other - Destantion ' ,fontsize=25)
axe3.set_xlabel('SimMobility ' , fontsize=25,labelpad=15)
axe3.set_ylabel('THS ', fontsize=25,labelpad=15)
axe3.set_xlim(0, 120000)
axe3.set_ylim(0, 120000)
axe3.text(10000, 80000, 'R-squared = %0.2f' % r2_score(Y1, Y1_pred),size=17)
axe3.text(10000, 90000, 'Coefficients = %0.2f' % linear_regressor1.coef_,size=17)
...
axe4.set_title('Linear regression - intermediate stops to Shop - Destantion ' ,fontsize=25)
...
axe4.text(20000, 4000, 'R-squared = %0.2f' % r2_score(Y1, Y1_pred),size=17)
axe4.text(20000, 3000, 'Coefficients = %0.2f' % linear_regressor1.coef_,size=17)
...
request = service.spreadsheets().values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range="Destenation-i-stop by zones!A2:D1000", valueInputOption="USER_ENTERED", body={"values":merge_stops_work_list}).execute()

request = service.spreadsheets().values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range="Destenation-i-stop by zones!K2:M1000", valueInputOption="USER_ENTERED", body={"values":merge_stops_shop_list}).execute()


request = service.spreadsheets().values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range="Destenation-i-stop by zones!U2:W1000", valueInputOption="USER_ENTERED", body={"values":merge_stops_edu_list}).execute()

request = service.spreadsheets().values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range="Destenation-i-stop by zones!AE2:AG1000", valueInputOption="USER_ENTERED", body={"values":merge_stops_other_list}).execute()
